      ...
      <section className="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-3">
            {details.map((item) => (
              <article key={item.label} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#f97316]">{item.label}</p>
          ...
          <form className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6 lg:col-span-2">
            ...
            <button className="mt-4 rounded-full bg-[#f97316] px-6 py-2.5 text-sm font-bold text-white transition hover:brightness-110">
              Send Message
            </button>
          </form>
        </div>
