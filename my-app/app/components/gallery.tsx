        ...
        <div className="mx-auto w-full max-w-[1800px] pb-6 lg:pb-12">
          ...
          >
            {galleryItems.map((item, index) => (
              <button
                ...
                className="relative h-full border-0 p-0 transition-[flex-grow] duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-[flex-grow] focus:outline-none"
                style={{
                  flexGrow:
                  ...
                  transition: 'transform 900ms ease, flex-grow 1000ms cubic-bezier(0.22,1,0.36,1)',
                }}
          ...
          <div className="no-scrollbar -mx-1 flex snap-x snap-mandatory gap-3 overflow-x-auto px-1 lg:hidden">
            {galleryItems.map((item, index) => (
              <div
                key={item.src}
                className="h-[52vh] min-h-[320px] w-[85vw] shrink-0 snap-center overflow-hidden rounded-2xl border border-zinc-800 shadow-[0_16px_28px_rgba(0,0,0,0.35)]"
                style={{
                  backgroundImage: `url(${item.src})`,
                  backgroundPosition: 'center',
                  backgroundSize: 'cover',
