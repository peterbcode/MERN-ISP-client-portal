...
const galleryItems = [
  { src: '/gallery/RBV-Dish.jpeg',      alt: 'Wireless dish installation on a Riebeek Valley property', tag: 'Installation',  caption: 'Precision wireless links for homes and farms',      detail: 'Aligned for stable valley coverage' },
  { src: '/gallery/High-speed.png',     alt: 'High-speed internet networking equipment',                tag: 'Network',       caption: 'High-speed equipment installed and tuned',          detail: 'Built for clean throughput' },
  { src: '/gallery/solar-and-dish.jpeg',alt: 'Solar-backed dish installation',                         tag: 'Off-grid',      caption: 'Solar-backed connectivity in the field',            detail: 'Power-conscious site deployments' },
  { src: '/gallery/storefront.png',     alt: 'Valley Computers storefront',                            tag: 'Support',       caption: 'Local support with a real storefront',              detail: 'Riebeek-Kasteel service desk' },
  { src: '/gallery/Rainbow.png',        alt: 'Rainbow over the Riebeek Valley service area',           tag: 'Coverage',      caption: 'Coverage across the Riebeek Valley',               detail: 'Local network, local terrain knowledge' },
  { src: '/gallery/tower-view.jpeg',    alt: 'Communication tower view over the valley',               tag: 'Infrastructure', caption: 'Strategic tower placement for valley coverage',   detail: 'Elevated vantage points for reliable links' },
  { src: '/gallery/installation.jpeg',  alt: 'Professional installation work in progress',             tag: 'Installation',  caption: 'Expert installation by trained technicians',        detail: 'Clean, professional setup every time' },
]
...
function GalleryCard({
  ...
  const minH    = isHero ? '430px' : '210px'
  const colW    = isHero ? '380px' : '280px'
  ...
  return (
    <article
      ...
      style={{ minHeight: minH, width: colW, gridRow: `span ${rowSpan}` }}
    >
      <Image
        ref={imgRef}
        src={item.src}
        ...
        className={`absolute inset-0 h-full w-full object-cover transition-[transform,filter] duration-700 ease-[cubic-bezier(.16,1,.3,1)] group-hover:brightness-105 ${isHero ? 'object-top' : 'object-center'}`}
      />
...
export default function Gallery() {
  ...
  return (
    <section id="gallerySection" className="relative scroll-mt-28 overflow-hidden py-20 text-white sm:py-24">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <AnimatedSection direction="up" delay={120}>
          {/* ── SCROLL OUTER: hides overflow, applies fade mask ── */}
          <div
            ref={scrollRef}
          ...
          >
            {/* ── TRACK: 3 fixed rows, auto columns flowing left→right ── */}
            <div
              style={{
                display: 'grid',
                ...
                gap: '12px',
                width: 'max-content',    /* CRITICAL: must be max-content or track won't extend */
                paddingBottom: '4px',
              }}
            >
